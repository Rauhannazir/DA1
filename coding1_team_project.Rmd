...
# price of coke
  a <- mean( dfc$price_in_huf , na.rm = T )
  ggplot(data=dfc)+
  geom_histogram(aes(x=price_in_huf), fill ='navyblue',color= "black" ,binwidth = 20)+
    labs(x='Price of Coke',
         y='Frequency of Count ')+
    ggtitle("Price Comparison")+
    theme_solarized()+
    theme(plot.title=element_text(size= 15,hjust=0.5))+
    scale_x_continuous( limits = c( 200 , 400 ) )+
  geom_segment( aes(x = mean( dfc$price_in_huf , na.rm = T ), y = 0, 
                    xend = mean( dfc$price_in_huf , na.rm = T ) , yend = 10) , color = 'brown')+
    annotate( "text" , x = mean( dfc$price_in_huf, na.rm = T ) + 20 , y = 9  , label = 'Mean' , color = 'brown')
  ...
  b <- mean( dfn$price_in_huf , na.rm = T )
  ggplot(data=dfn)+
    geom_histogram(aes(x=price_in_huf), fill ='navyblue',color= "black" ,binwidth = 20)+
    labs(x='Price of Nutella',
         y='Frequency of Count ')+
    ggtitle("Price Comparison")+
    theme_solarized()+
    theme(plot.title=element_text(size= 15,hjust=0.5))+
    scale_x_continuous( limits = c( 1060 , 1450 ) )+
    scale_y_continuous(limits = c(0,8.5))+
    geom_segment( aes(x = mean( dfn$price_in_huf , na.rm = T ), y = 0, 
                      xend = mean( dfn$price_in_huf , na.rm = T ) , yend = 8.5) , color = 'brown')+
    annotate( "text" , x = mean( dfn$price_in_huf, na.rm = T ) + 32 , y = 8.5  , label = 'Mean' , color = 'brown')
